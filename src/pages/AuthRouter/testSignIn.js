  ...
  const login = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Si la réponse est une erreur (code 4xx ou 5xx)
        if (data.message) {
        ...
        if (data.errors) {
          if (data.errors.email) {
            newErrors.email = data.errors.email.join(', ');
          }
          if (data.errors.password) {
            newErrors.password = data.errors.password.join(', ');
          }
        } else if (data.error) {
          // Par exemple, pour une erreur générale
            ...
            <form onSubmit={login} className="flex flex-col gap-4">
              <div className="mb-4">
                ...
                <input
                  ...
                  className={`w-full px-4 py-4 bg-[#141314] rounded-sm text-white focus:outline-none focus:ring-2 focus:ring-orange-400 ${
                    errors.email ? 'border border-red-500 focus:ring-0' : ''
                  }`}
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  id="password"
                  className={`w-full px-4 py-4 bg-[#141314] rounded-sm text-white focus:outline-none focus:ring-2 focus:ring-orange-400 ${
                    errors.password ? 'border border-red-500 focus:ring-0' : ''
                  }`}
...
//                         setErrors(prev => ({ ...prev, email: "Utilisateur non trouvé avec cet email." }));
...
//                                 {loading ? (
//                                     <div className="flex items-center">
//                                         <svg className="w-6 h-6 text-white animate-spin mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
